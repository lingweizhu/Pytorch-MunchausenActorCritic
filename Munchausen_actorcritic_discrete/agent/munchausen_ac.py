...
from Munchausen_actorcritic_discrete.model import TwinnedQNetwork, CateoricalPolicy
from Munchausen_actorcritic_discrete.utils import disable_gradients


class MunchausenACAgent(BaseAgent):
    ...
    def __init__(self, env, test_env, log_dir, num_steps=100000, batch_size=64,
                 ...
                 sigma=0.2, start_steps=20000, alpha = 0.9,
        ...
        super().__init__(
            ...
            multi_step, sigma, start_steps, update_interval,
    ...
    def calc_target_q(self, states, actions, rewards, next_states, dones):
        with torch.no_grad():
            ...
            _, _, log_curr_action_probs = self.policy.sample(states)
            next_q1, next_q2 = self.target_critic(next_states)
            next_q = (action_probs * (
                torch.min(next_q1, next_q2) - self.sigma * log_action_probs
                )).sum(dim=1, keepdim=True)
            munchausen_term = self.alpha * self.sigma * log_curr_action_probs
        assert rewards.shape == next_q.shape
        return rewards + munchausen_term + (1.0 - dones) * self.gamma_n * next_q
    ...
    def calc_policy_loss(self, batch, weights):
        ...
        # Policy objective is maximization of (Q + sigma * entropy) with
        # priority weights.
        policy_loss = (weights * (- q - self.sigma * entropies)).mean()
